...
void Parser::statement()
{
	...
	if(see(T_IDENTIFIER)) {
		...
		if (match(T_QUESTION)) {
			...
			expression();
			mustBe(T_COLON);
			//Чтобы не выполнять блок ELSE в случае выполнения первого выражения, 
...
void Parser::factor()
{
	...
	*/
	if(see(T_NUMBER)) {
		...
		relation();
		mustBe(T_RPAREN);
		//Если встретили открывающую скобку, тогда следом может идти любое арифметическое выражение или логическое и обязательно
		//закрывающая скобка.
	}
